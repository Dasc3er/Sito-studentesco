...
if (!$options["debug"]) {
    ...
    if (isAdminUserAutenticate()) {
        ...
        $app->get('/presente/:id/:corso', 
                function ($id, $corso) use($options, $app) {
                    $app->render('corsi.php', 
                            array ('options' => $options, 'presente' => true, 'persona' => $id, 'corso' => $corso));
        ...
        $app->get('/ra', 
                function () use($options, $app) {
                    $app->render('pdf/classes.php', array ('options' => $options, "download" => true));
                    $file = "random.txt";
                    $res = $app->response();
                    $res['Content-Description'] = 'File Transfer';
                    $res['Content-Type'] = 'application/octet-stream';
        ...
        $app->get('/sospendi/:id', 
                function ($id) use($options, $app) {
                    $app->render('aule.php', array ('options' => $options, 'cambia' => $id));
                    $app->redirect($app->urlFor('aule') . '#blocked');
                });
    ...
    if (isUserAutenticate() && !$options["first"] && $options["autogestione"] != null) {
        ...
        $app->map('/barcode', 
                function () use($options, $app) {
                    $app->render('login/.php', array ('options' => $options, 'info' => true));
                    if (fatto()) $app->redirect($app->urlFor('index'));
                })->via('GET', 'POST');
        ...
        $app->get('/pdf', 
                function () use($options, $app) {
                    $app->response->headers->set('Content-Type', 'application/pdf');
                    $app->render('pdf/user.php', array ('options' => $options));
                });
        ...
        if (isAdminUserAutenticate()) {
            ...
            $app->get('/blocca/:id', 
                    function ($id) use($options, $app) {
                        $app->render('proposte.php', array ('options' => $options, 'cambia' => $id));
                        if (!isset($_GET["ajax"])) $$app->redirect($app->urlFor('proposte') . '#blocked');
                    });
            
            $app->get('/cambia/:id', 
                    function ($id) use($options, $app) {
                        $app->render('citazioni.php', array ('options' => $options, 'cambia' => $id));
                        if (!isset($_GET["ajax"])) $$app->redirect($app->urlFor('citazioni') . '#blocked');
                    });
        }
    }
