    ...
    if (isUserAutenticate() && !$dati['first'] && $dati['autogestione'] != null) {
        ...
        if ($dati['sezioni']['citazioni']) {
            ...
            $app->get('/citazioni/:id', 
                    function ($id) use($dati, $app) {
                        $app->render('citazioni.php', array ('dati' => $dati, 'citazione' => true, 'id' => $id));
                        if (!isset($_GET['ajax'])) {
            ...
            $app->get('/citazione/:id', 
                    function ($id) use($dati, $app) {
                        $app->render('citazioni.php', array ('dati' => $dati, 'citazione' => true, 'view' => $id));
                    });
            
            $app->map('/citazione', 
                    function () use($dati, $app) {
                        $app->render('citazioni.php', array ('dati' => $dati, 'citazione' => true, 'new' => true));
                        if (fatto()) {
                            $app->redirect($app->urlFor('citazioni'));
                        }
                    })->via('GET', 'POST');
        ...
        if ($dati['sezioni']['aule']) {
            ...
            $app->get('/aule/:id', 
                    function ($id) use($dati, $app) {
                        $app->render('aule.php', array ('dati' => $dati, 'aula' => true, 'id' => $id));
                        if (!isset($_GET['ajax'])) {
            ...
            $app->get('/aula/:id', 
                    function ($id) use($dati, $app) {
                        $app->render('aule.php', array ('dati' => $dati, 'aula' => true, 'view' => $id));
                    });
            
            $app->map('/aula', 
                    function () use($dati, $app) {
                        $app->render('aule.php', array ('dati' => $dati, 'aula' => true, 'new' => true));
                        if (fatto()) {
                            $app->redirect($app->urlFor('aule'));
                        }
                    })->via('GET', 'POST');
