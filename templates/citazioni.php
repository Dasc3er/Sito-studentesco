...
if (isset($rifiuta)) {
    ...
    if (!$error) {
        ...
        echo '</textarea></div>
                        <div class="form-group">
                            ...
                            <div class="col-xs-6">
                                <a href="' .
                 $dati['info']['root'] . 'citazioni" class="btn btn-default btn-block">Annulla</a>
                            </div>
                        </div>
                    </form>
                </div>
    ...
    if ($dati['database']->count("voti", array ("AND" => array ("persona" => $dati["user"], "citazione" => $id, "stato" => 1))) != 0) $dati['database']->update(
            "voti", array ("stato" => 0), array ("AND" => array ("persona" => $dati["user"], "citazione" => $id)));
    ...
    echo '><span id="nome">Tutti i professori</span> <span class="badge">' . $dati['database']->count("citazioni", array ("stato" => 0)) .
