    ...
    echo '
            <div class="jumbotron">
                <div class="container text-center">
                    ...
                    <p><a class="btn btn-success" href="' . $options["root"] . 'barcode/' . $id . '" target="_blank">Ottieni barcode <i class="fa fa-chevron-down"></i></a></p>
                </div>
            </div>
            <hr>
            <div class="container">';
    ...
    if ($results != null) {
        foreach ($results as $result) {
            ...
            if ($corso != - 1) {
                echo '
                    <section>
                        <p class="title"><a href="' . $options["root"] . 'corso/' . $corsi[$corso]["id"] . '">' . $corsi[$corso]["nome"] . '</a>';
                echo '</p>
                ...
                if ($id == $options["user"] && time($options["database"])) {
                    ...
                    if ($squadra == null) echo '
                                <a id="squad" href="' . $options["root"] . 'squadra" class="btn btn-primary btn-block btn-lg">Crea squadra</a>';
                    else echo '
                                <a id="squad" href="' . $options["root"] . 'squadra/' . $squadra . '" class="btn btn-primary btn-block btn-lg">Gestisci squadra</a>';
                    echo '
                        <a href="' . $options["root"] . 'corsi/' . $corsi[$corso]["id"] . '" class="btn btn-danger btn-block btn-lg">Elimina iscrizione</a>';
                }
                echo '
                    </section>';
            }
