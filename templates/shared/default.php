...
function id($database) {
    ...
    if (isUserAutenticate()) {
        $results = $database->select("persone", array(
            "id"
        ), array(
...
function isUserFree($database, $username, $user = "") {
    return ($database->count("persone", array(
        "AND" => array(
            "username" => $username,
            "id[!]" => $user
        )
...
function isAdmin($database, $id) {
    return ($database->count("admins", array(
        "id" => $id
    )) != 0);
}
...
function iscritto($database, $id, $user) {
    return ($database->count("iscrizioni", array(
        "AND" => array(
            "persona" => $user,
            "corso" => $id,
            "stato" => 0
...
function occupato($database, $id, $user) {
    $when = $database->get("corsi", "quando", array(
        "AND" => array(
            "id" => $id,
            "stato" => 0
        )
    ...
    $result = false;
    $datas = $database->select("iscrizioni", "*", array(
        "AND" => array(
            "persona" => $user,
            "stato" => 0
        )
    ...
    if ($datas != null) {
        foreach ($datas as $data) {
            $quando = $database->get("corsi", "quando", array(
                "AND" => array(
                    "id" => $data["corso"],
                    "stato" => 0
                )
...
function pieno($database, $id) {
    ...
    )) >= $database->get("corsi", "max", array(
        "id" => $id
    )));
}
// Controlla se c'� ancora tempo per iscriversi ai corsi
function tempo($database) {
    return ($database->count("autogestioni", array(
        "AND" => array(
            "id" => $database->max("autogestioni", "id"),
            "#ultima[>=]" => "CURDATE()"
        )
    )) != 0);
}
...
function ntorneo($database, $user) {
    return ($database->get("corsi", "id", array(
        "AND" => array(
            "quando" => "1,2,3,4,5",
            "id" => $database->get("iscrizioni", "corso", array(
                "AND" => array(
                    "persona" => $user,
                    "stato" => 0
                )
...
function voti($database, $id, $user) {
    return ($database->count("voti", array(
        "AND" => array(
            "persona" => $user,
            "citazione" => $id,
            "stato" => 0
...
function like($database, $id, $user) {
    return ($database->count("like", array(
        "AND" => array(
            "persona" => $user,
            "corso" => $id
        )
...
function proposta($database, $autogestione, $user) {
    return ($database->count("corsi", array(
        "AND" => array(
            "autogestione" => $autogestione,
            "creatore" => $user,
            "quando" => null
        )
    )) <= 3 && classe($database, $user));
}
// Restituisce l'identificativo della scuola dell'utente
function scuola($database, $user) {
    return $database->get("classi", "scuola", array(
        "id" => $database->get("studenti", "classe", array(
            "AND" => array(
                "id" => $database->max("studenti", "id"),
                "persona" => $user
            )
        ))
    ));
...
function scuolagiusta($database, $corso, $user) {
    return (scuola($database, $user) == $database->get("corsi", "scuola", array(
        "id" => $corso
    )));
}
// Controlla se l'utente � inserito in una classe
function classe($database, $user) {
    return ($database->count("studenti", array(
        "AND" => array(
            "id" => $database->max("studenti", "id"),
            "persona" => $user
        )
...
function full($database, $id) {
    return ($database->count("pomeriggio", array(
        "AND" => array(
            "aula" => $id,
            "stato" => 0
        )
    )) >= $database->get("aule", "max", array(
        "id" => $id
    )));
}
...
function tempopomeriggio($database, $id) {
    return ($database->count("aule", array(
        "AND" => array(
            "id" => $id,
            "#data[>]" => "CURDATE()"
        )
