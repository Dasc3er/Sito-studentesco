...
if (isAdminUserAutenticate() && isset($reset) && !isAdmin($options["database"], $id)) {
    ...
    while ($options["database"]->count("persone", array ("AND" => array ("username" => $username, "id[!]" => $reset))) != 0)
        $username .= rand(0, 999);
    $options["database"]->update("persone", array ("username" => $username, "password" => $password, "email" => "", "stato" => 0), 
    ...
    $admins = $options["database"]->select("admins", "*", array ("ORDER" => "id"));
    ...
    $results = $options["database"]->select("persone", "*", array ("ORDER" => "nome"));
