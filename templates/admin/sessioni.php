...
if (isset($reset)) $dati['database']->query("TRUNCATE TABLE sessioni");
$datatable = true;
$pageTitle = "Visite al sito";
require_once 'templates/shared/header.php';
$results = $dati['database']->select("sessioni", "*", array ("ORDER" => "data DESC"));
echo '
...
echo '
                    ...
                    <p>Visite al sito: ' . $dati['database']->count("sessioni", "*") . '</p>
                    <p><a href="' . $dati['info']['root'] . 'reset/sessioni" class="btn btn-danger">Azzera il registro delle sessioni</a></p>
                </div>
            </div>';
